...
Nach [[Gruppen#Theorem von Cauchy|Theorem von Cauchy]] gilt es existiert ein $x \in Z(G)$ mit $\text{ord}(g)=p$. Sei $K:=\langle g\rangle$ die erzeugte  [[Gruppen#Zyklische Gruppen|zyklische Untergruppe]] von $G$ von [[Gruppen#Ordnung|Ordnung]] $p$.  Dann gilt $K\subseteq Z(G)$ kommutiert mit allen Elementen aus $G$. Insbesondere ist dann auch [[Gruppenwirkung#Rechts und Linksnebenklassen|$gK=Kg$]] für jedes $g\in G$ und damit [[Normalteiler#Äquivalente Formulierungen|$K \trianglelefteq G$]]. Außerdem gilt nach [[Gruppenwirkung#Satz von Lagrange|Satz von Lagrange]], dass
...
Betrachte die Wirkung durch Konjugation von $\text{Syl}_{p}(G)\ni H_{p} \subset G \curvearrowright \text{Syl}_{p}(G)$. Dann [[Gruppenwirkung#Bahn und Stabilisator#Eigenschaften|partitionieren]] die Bahnen $G$, also gilt$$n_{p}=\lvert \text{Syl}_{p}(G)\rvert=\sum\limits_{\begin{align}B \cdot x \text{ Bahn von}\\H_{p} \text{ auf }\text{Syl}_{p}(G)\end{align}}\lvert B \cdot x\rvert$$Da mit der [[Gruppenwirkung#Bahnenformel|Bahnenformel]] für die Bahnen $B\cdot x$ gilt $\lvert B\cdot x\rvert=\frac{\lvert H_p\rvert}{\lvert B_{x}\rvert}$ eine Potenz von $q$ oder $1$. Damit ist also $$\begin{aligned}n_{p}&=\sum\limits_{\begin{align}B \cdot x \text{ Bahn von}\\H_{p} \text{ auf }\text{Syl}_{p}(G)\end{align}}\lvert B \cdot x\rvert \\&\equiv \#\set{\text{Bahnen mit 1 Element}} \mod p\end{aligned}$$Insbesondere ist die Bahn von $H_{p}\in \text{Syl}_{p}(G)$ unter der Wirkung von $H_{p}$ nur bestehend aus $H_{p}$ d.h. es gibt mindesten eine solche Bahn mit einem Element.

Es verbleibt nun zu zeigen, dass es keine weitere gibt.
*Annahme* es existiert ein $H_{p} \neq H \in \text{Syl}_{p}(G)$, sodass $\forall x \in H_{p}$ gilt $xHx^{-1}=H$ also, dass für den [[Normalisator]] $N_{G}(H)$ gilt, dass $H_{q} < N_{H}$ eine Untergruppe davon ist. 

Insbesondere ist $H \trianglelefteq N_{G}(H)$ also die Projektion $\pi: N_{G}(H) \rightarrow N_{G}(H)/H$ ein [[Gruppenhomomorphismen|Homorphismus]] und es gilt $\ker \pi = H \neq H_p$. Daraus folgt, dass $\set{e} \subsetneq \pi(H_{p})<N_{G}(H)/H$. Betrachte nun $$K:=\pi^{-1}(\pi(H_{p}))\leq N_{G}(H)$$ Wegen dem [[Faktorgruppen#Isomorphiesatz|Isomorphiesatz]] und da $\pi\vert_{K}$ auch $\ker \pi = H \subseteq K$ enthält, gilt mit [[Gruppenwirkung#Satz von Lagrange|Lagrange]] $$\lvert K\rvert=\lvert H\rvert\cdot \lvert \pi(K)\rvert=\lvert H\rvert\cdot \lvert \pi(H_{p})\rvert$$Da $\pi(H_{q})$ eine nicht-triviale Untergruppe von $N_{G}(H)/H$ ist folgt auch mit Lagrange, dass $\lvert \pi(H_{p})\rvert=p^{s}, s\geq 1$ ist. Mit $\lvert H\rvert=p^{a}$ ist also $K$ eine Untergruppe von $G$ die Ordnung $p^{s+a}$ hat. Aber $p^{a}$ war mit maximalem $a$ gewählt sodass $p^{a}\mid \lvert G\rvert$. Wir haben also einen *Widerspruch*.  
